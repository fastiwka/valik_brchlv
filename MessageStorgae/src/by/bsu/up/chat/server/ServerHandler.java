...
public class ServerHandler implements HttpHandler {
        ...
        try {
            ...
            if (index > messageStorage.size()) {
                return Response.badRequest(
                        String.format("Incorrect request token: %s. Server does not have so many messages", token));
            }
    ...
    private Response doPost(HttpExchange httpExchange) {
        try {
            ...
            logger.info(String.format("New message from user recieved: %s", message));
            ...
            logger.error("Message with same id exists.", e);
            return new Response(Constants.RESPONSE_CODE_BAD_REQUEST, "Message with same id exists");
        }
    }
    ...
    private Response doPut(HttpExchange httpExchange) {
        try {
            Message message = MessageHelper.getClientMessage(httpExchange.getRequestBody());
            logger.info(String.format("Edited message from user recieved: %s", message));
            if (messageStorage.updateMessage(message)) {
                return Response.ok();
            }
            return Response.badRequest("This message does not exist");
        } catch (ParseException e) {
            logger.error("Unable parse message.", e);
            return new Response(Constants.RESPONSE_CODE_BAD_REQUEST, "Incorrect request body");
        }
    }
